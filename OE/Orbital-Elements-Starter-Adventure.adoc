...
[NOTE]
====
Space Transportation Code 1192b

A vehicle or structure that is left in a stable orbit and that poses no likely threat to others is classified as abandoned. The owner of said vehicle or structure is not considered to hold responsiblity for an abandoned vehicle or structure which immediately becomes classified as salvage unless owner asserts ownership and continues to hold legal responsibility

A vehicle that may pose a likely threat 
====
...
____
*XO:* All you need to do is check out the transporter for survivors. Nobody on the ship is communicating now. There's a chance the transporter was abandoned or maybe the crew are awaiting rescue. Or they might all be dead from a radiation leak, but if that happended we would surely detect it from your suit sensors the moment that you enter.
____
...
====
.Wine-only 
Alcoholic drinks are dehydrating, they remove water from your cells, but wine is 90% water, so wine will on balance hydrate you, so you won't die of thirst drinking only wine, at least not as quickly as drinking nothing, or -- it has to be said -- drinking your pee, which will likely only extend your surival from 3 days to 4. As a bonus, red wine also contains some nutrients and calories, so it substitutes for a bit of food. Good to know, eh!
====
